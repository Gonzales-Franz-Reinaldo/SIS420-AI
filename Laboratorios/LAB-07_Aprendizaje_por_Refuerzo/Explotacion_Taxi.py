...
def train(episodes, save_q_table=True, save_plot=True):
    ...
    if save_plot:
        plt.savefig('recompensas_qlearning.png')
    plt.show()

    # Guardar la tabla Q en un archivo pkl
    if save_q_table:
        with open('taxi_qlearning.pkl', 'wb') as f:
            pickle.dump(q_table, f)
        print("Tabla Q guardada en 'taxi_qlearning.pkl'.")

def exploit(q_table_file='taxi_qlearning.pkl', num_runs=10):
