...
def main():
    ...
    # Inicializar la tabla Q con ceros y el conteo de acciones con unos
    ...
    action_counts = np.ones((state_size, action_size))  # Inicializar en 1 para evitar división por cero
    ...
    # Hiperparámetros
    c = 1  # Constante que controla el nivel de exploración
    learning_rate = 0.9  # Tasa de aprendizaje
    discount_rate = 0.8  # Factor de descuento

    total_steps = 0  # Contador total de pasos
    ...
    for episode in range(EPISODES):
        ...
        for step in range(STEPS_PER_EPISODE):
            total_steps += 1  # Incrementar el contador total de pasos

            # Elegir una acción usando UCB
            action_values = qtable[state, :] + c * np.sqrt(np.log(total_steps + 1) / action_counts[state, :])
            ...
            # Actualizar el conteo de la acción seleccionada
            action_counts[state, action] += 1

            # Actualizar la tabla Q usando la fórmula Q-learning
